...
const plannedPosts = [
  {
    date: "Poniedziałek",
    platform: "LinkedIn",
    title: "5 błędów przy wdrażaniu AI w content marketingu",
    status: "Szkic",
  },
  {
    date: "Wtorek",
    platform: "Instagram",
    title: "Reels: kulisy pracy nad kampanią",
    status: "Do nagrania",
  },
  {
    date: "Czwartek",
    platform: "Blog",
    title: "Jak analizować skuteczność contentu?",
    status: "Do SEO",
  },
  {
    date: "Piątek",
    platform: "TikTok",
    title: "Krótki poradnik dla twórców",
    status: "Zaplanowane",
  },
];
...
const collaborations = [
  ...
  {
    brand: "EduTech",
    scope: "1 post LinkedIn + newsletter",
    deadline: "18 czerwca",
    payment: "1800 zł",
...
const integrations = [
  ...
  },
  {
    name: "YouTube",
    status: "Priorytet",
    note: "Filmy, Shorts, analityka kanału",
  },
  {
    name: "LinkedIn",
    status: "Planowane",
    note: "Strony firmowe i B2B content",
  },
  {
    name: "TikTok",
    status: "Później",
    note: "Tryb półautomatyczny / API",
  },
  {
    name: "Blog / WordPress",
    status: "Planowane",
    note: "Artykuły, SEO, publikacja",
  },
  {
    name: "Google Analytics",
    status: "Planowane",
    note: "Ruch, konwersje, źródła",
  },
];
      ...
      <div className="grid min-h-screen lg:grid-cols-[280px_1fr]">
        ...
        <aside className="border-r border-white/10 bg-white/[0.03] p-5">
          ...
          <div className="mt-8 rounded-3xl border border-white/10 bg-white/[0.04] p-4">
            <p className="text-sm font-bold">Plan rozwoju</p>
            <p className="mt-2 text-xs leading-5 text-white/50">
              Integracje, AI analiza i publikowanie będziemy podpinać moduł po
              module.
            </p>
          </div>
        </aside>
        ...
        <section className="p-6 lg:p-10">
          ...
          <header className="mb-8 flex flex-col justify-between gap-4 rounded-[2rem] border border-white/10 bg-white/[0.04] p-6 md:flex-row md:items-center">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-black capitalize md:text-4xl">
            ...
            <div className="flex flex-wrap gap-3">
              <Link
                href="/dashboard"
                className="rounded-2xl border border-white/10 px-5 py-3 text-sm font-bold text-white/70 hover:bg-white/10"
              >
                Dashboard
              </Link>
              ...
              <button
                ...
                disabled={signingOut}
                className="rounded-2xl border border-red-400/30 bg-red-400/10 px-5 py-3 text-sm font-bold text-red-200 hover:bg-red-400/20 disabled:cursor-not-allowed disabled:opacity-60"
              >
                ...
                <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
                  ...
                  <div className="mt-5 space-y-3">
                    {aiInsights.map((insight) => (
                      <div
                        key={insight}
                        className="rounded-2xl bg-white/[0.05] p-4 text-sm text-white/70"
                      >
                        {insight}
                      </div>
                    ))}
              ...
              <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
                ...
                <div className="mt-6 space-y-4">
                  <input
                    ...
                    placeholder="Temat contentu"
                  />

                  <select className="w-full rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-3 outline-none focus:border-cyan-300">
                  ...
                  <textarea
                    className="min-h-32 w-full rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-3 outline-none placeholder:text-white/30 focus:border-cyan-300"
                    placeholder="Cel, grupa odbiorców, styl, CTA..."
                  />
              ...
              <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
                ...
                <div className="mt-5 rounded-3xl bg-[#0e1024] p-6">
                  ...
                  <p className="mt-4 text-sm leading-6 text-white/65">
                    Większość firm publikuje regularnie, ale nadal nie wie,
                    które treści naprawdę działają. Problem nie leży w samej
                    częstotliwości, tylko w braku analizy: tematu, CTA, formatu
            ...
            <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
              ...
              <div className="mt-6 space-y-3">
                {plannedPosts.map((post) => (
                  <div
                    key={post.title}
                    className="grid gap-3 rounded-2xl bg-white/[0.05] p-4 md:grid-cols-[140px_140px_1fr_140px]"
                  >
          ...
          {activeTab === "AI Analiza" && (
            <div className="grid gap-6 lg:grid-cols-3">
              {["Content Score", "Predykcja wyniku", "Rekomendacje"].map(
                (item) => (
                  <div
                    key={item}
                  ...
                  >
                    ...
                    <p className="mt-4 text-sm leading-6 text-white/60">
                      AI ocenia jakość hooka, CTA, styl, dopasowanie do
                      platformy i potencjał zaangażowania.
                    </p>
                  </div>
                )
              )}
          ...
          {activeTab === "Influencer Planner" && (
            <div className="space-y-6">
              <div className="grid gap-4 md:grid-cols-4">
                {["Briefy", "Deadline’y", "Płatności", "Media Kit"].map(
                  (item) => (
                    <div
                      key={item}
                      className="rounded-3xl border border-white/10 bg-white/[0.04] p-5"
              ...
              <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-3">
                  {collaborations.map((item) => (
                    <div
                      key={item.brand}
                      className="rounded-3xl bg-white/[0.05] p-5"
                    >
                      ...
                      <p className="mt-1 text-sm text-white/50">
                        Wynagrodzenie: {item.payment}
                      </p>
                      <p className="mt-4 rounded-full bg-white/[0.08] px-4 py-2 text-sm">
                        {item.status}
                      </p>
                    </div>
                  ))}
          ...
          {activeTab === "Konkurencja" && (
            <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
              ...
              <p className="mt-2 max-w-2xl text-sm text-white/55">
                Dodawaj publiczne treści konkurencji, porównuj tematy, formaty,
                CTA i luki contentowe.
              </p>
                ...
                {[
                  ...
                  <div key={item} className="rounded-3xl bg-white/[0.05] p-5">
                    ...
                    <p className="mt-3 text-sm text-white/55">
                      Analiza publicznych danych i ręcznie dodanych publikacji
                      konkurencji.
                    </p>
                  </div>
                ))}
              ...
              {[
                ...
                >
                  ...
                  <p className="mt-3 text-sm leading-6 text-white/60">
                    Moduł do tworzenia, analizy i recyklingu treści blogowych w
                    wielu kanałach.
                  </p>
                </div>
              ))}
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
                ...
                <p className="mt-3 text-sm text-white/60">
                  Ton marki, słowa preferowane, słowa zakazane, persony i styl
                  komunikacji.
                </p>
              </div>
              ...
              <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
                ...
                <p className="mt-3 text-sm text-white/60">
                  Ustawienia projektu, członkowie zespołu, role i dostęp do
                  integracji.
                </p>
              </div>
            </div>
