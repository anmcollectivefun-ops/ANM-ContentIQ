...
async function logMetaSelectionDiagnostics(
  ...
  const grantedFromPermissions =
    (((permissions as { data?: MetaPermission[] } | null)?.data || [])
      .filter((item) => item.status === "granted")
      .map((item) => item.permission)
      .filter(Boolean)) as string[];

  const grantedFromDebug =
    (((tokenDebug as { data?: { scopes?: string[] } } | null)?.data?.scopes ||
      [])
      .filter(Boolean)) as string[];

  const grantedScopes = Array.from(
    new Set([...grantedFromPermissions, ...grantedFromDebug])
  ).sort();
  ...
  console.error(
    `[Meta page selection diagnostics][${platform}]`,
    JSON.stringify(
      {
        reason,
        user,
...
async function fetchPageAccessToken(
  ...
  if (data.error) {
    await logMetaSelectionDiagnostics(
      platform,
      userToken,
      data,
      "/me/accounts with access_token field error"
    );
  ...
  if (!page?.access_token) {
    await logMetaSelectionDiagnostics(
      ...
      userToken,
      data,
      `Selected page ${pageId} missing page access token`
    );

    throw new Error(
      "Nie znaleziono tokenu wybranej strony. Sprawdź dostęp do tej strony i pages_show_list."
    );
  }
...
async function fetchInstagramFromPage(pageId: string, pageToken: string) {
  ...
  if (!igId) {
    throw new Error(
      "Wybrana Facebook Page nie ma podpiętego Instagram Business Account."
    );
  }
...
async function upsertConnection(
  ...
  const connectionData = {
    ...
    login_config_id: pending.login_config_id ?? null,
    token_source: pending.token_source ?? null,
...
export async function GET(req: NextRequest) {
  ...
  if (!pending) {
    return NextResponse.json(
      { error: "Brak oczekującego wyboru strony Meta." },
      { status: 404 }
    );
  }
...
export async function POST(req: NextRequest) {
  ...
  if (!pageId || !pending.pages.some((page) => page.id === pageId)) {
    return NextResponse.json(
      { error: "Nieprawidłowa strona Meta." },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    if (pending.platform === "facebook") {
      await upsertConnection(pending, {
        id: page.id,
        name: page.name,
    ...
    response.cookies.set("ciq_meta_page_selection", "", {
      path: "/",
      maxAge: 0,
    });
